    ...
    <div class="mx-auto max-w-5xl px-4 py-6">
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200">
            ...
            <div class="p-5 space-y-4">
                <form method="GET" action="{{ route('retirada.autorizacao.index') }}"
                    class="grid gap-3 rounded-2xl border bg-gray-50 p-4 md:grid-cols-3">
                    ...
                    <div class="flex gap-2">
                        <button type="submit"
                            class="flex-1 rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white">
                            Filtrar
                        </button>
                        <a href="{{ route('retirada.autorizacao.index') }}"
                            class="flex-1 rounded-xl border border-gray-300 px-4 py-3 text-center text-sm font-semibold text-gray-700">
                            Limpar
                        </a>
                    </div>
                </form>
                        ...
                        <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-gray-600">
                                        Solicitado por: <span
                                            class="font-medium text-gray-800">{{ $r->user->name ?? '-' }}</span>
                                    </p>
                                </div>
                            ...
                            <div class="mt-2 rounded-xl border border-gray-100 bg-white px-3 py-2 text-xs text-gray-600">
                                Autorizado por: <span
                                    class="font-medium text-gray-800">{{ $r->retirada_autorizada_por ?? '-' }}</span>
                            </div>
                            ...
                            @if ($r->status === 'pendente')
                                <div class="mt-4 grid grid-cols-3 gap-2">
                                    <form action="{{ route('retirada.autorizacao', $r->id) }}" method="POST">
                                        ...
                                        <button type="submit"
                                            class="w-full rounded-xl bg-green-400 px-4 py-3 text-sm font-bold text-white">
                                            Autorizar
                                        </button>
                                    </form>
                                    ...
                                    <form action="{{ route('retirada.autorizacao', $r->id) }}" method="POST">
                                        ...
                                        <button type="submit"
                                            class="w-full rounded-xl bg-yellow-600 px-4 py-3 text-sm font-bold text-white">
                                            Negar
                                        </button>
                                    </form>

                                    <form action="{{ route('retirada.deletar', $r->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit"
                                            class="w-full rounded-xl bg-red-600 px-4 py-3 text-sm font-bold text-white">
                                            Deletar
                                        </button>
                                    </form>
                                </div>
                            @elseif ($r->status === 'cancelar entrega')
                                <div class="mt-4 grid grid-cols-2 gap-2">
                                    ...
                                    <form action="{{ route('retirada.cancelar', $r->id) }}" method="POST">
                                        ...
                                        @method('PUT')
                                        <button type="submit"
                                            class="w-full rounded-xl bg-green-600 px-4 py-3 text-sm font-bold text-white">
