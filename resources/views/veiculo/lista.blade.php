                        ...
                        <tbody>
                            @forelse($veiculos as $v)
                                <tr class="border-t">
                                    ...
                                    <td class="px-3 py-3">{{ $v->marca }} • {{ $v->modelo }} ({{ $v->ano }})
                                    </td>
                                    ...
                                    <td class="px-3 py-3">
                                        @if ($v->status === 'ativo')
                                            <span
                                                class="text-xs px-2 py-1 rounded-full bg-green-50 text-green-700">Ativo</span>
                                        @elseif($v->status === 'manutencao')
                                            <span
                                                class="text-xs px-2 py-1 rounded-full bg-yellow-50 text-yellow-700">Manutenção</span>
                                        @else
                                            <span
                                                class="text-xs px-2 py-1 rounded-full bg-gray-50 text-gray-700">{{ ucfirst($v->status) }}</span>
                                        @endif
                                    </td>
                                    <td class="px-3 py-3">
                                        @if ($v->veiculo_qr_code)
                                            <a href="{{ $v->qr_code_url }}" target="_blank" class="text-blue-600 underline">
                                                Ver QR Code
                                            </a>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-3 py-3">
                                        @if (!$v->veiculo_qr_code)
                                            <form action="{{ route('veiculo.qrcode.gerar', $v->id) }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                        ...
                                        @else
                                            <form action="{{ route('veiculo.qrcode.regerar', $v->id) }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                                    class="px-3 py-1 rounded-md bg-indigo-50 text-indigo-700 text-sm border">
                                                    Regerar QR Code
                                                </button>
                                            </form>
                                        @endif
                                    ...
                                    <td class="px-3 py-3">
                                        <div class="flex gap-2">
                                            <button type="button"
                                                ...
                                                data-id="{{ $v->id }}" data-placa="{{ $v->placa }}"
                                                data-marca="{{ $v->marca }}" data-modelo="{{ $v->modelo }}"
                                                data-ano="{{ $v->ano }}" data-cor="{{ $v->cor }}"
                                                data-tipo="{{ $v->tipo_veiculo_id }}"
                                                data-combustivel="{{ $v->combustivel }}" data-km="{{ $v->km_atual }}"
                                                data-status="{{ $v->status }}"
                                                data-update-route="{{ route('veiculo.edit', $v->id) }}">Alterar</button>

                                            <form action="{{ route('veiculo.destroy', $v->id) }}" method="POST"
                                                onsubmit="return confirmDelete(event, this, '{{ addslashes($v->placa) }}')">
                                                @csrf
                                                <button type="submit"
                ...
                @forelse($veiculos as $v)
                    <div class="border rounded-xl p-3">
                        <div class="flex items-start justify-between">
                            <div>
                                <div class="text-sm font-medium text-gray-800">{{ $v->placa }} — {{ $v->marca }}
                                    {{ $v->modelo }}</div>
                                <div class="text-xs text-gray-500 mt-1">{{ $v->tipoVeiculo->nome ?? '-' }} •
                                    {{ $v->ano }}</div>
                                <div class="text-xs text-gray-500 mt-1">KM:
                                    {{ number_format($v->km_atual ?? 0, 1, ',', '.') }}</div>
                            </div>
                            ...
                            <div class="flex flex-col items-end gap-2">
                                <button type="button"
                                    ...
                                    data-marca="{{ $v->marca }}" data-modelo="{{ $v->modelo }}"
                                    data-ano="{{ $v->ano }}" data-cor="{{ $v->cor }}"
                                    data-tipo="{{ $v->tipo_veiculo_id }}" data-combustivel="{{ $v->combustivel }}"
                                    data-km="{{ $v->km_atual }}" data-status="{{ $v->status }}"
                                    data-update-route="{{ route('veiculo.edit', $v->id) }}">Alterar</button>
            ...
            <form id="editVehicleForm" method="POST" class="px-6 py-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <label class="block">
                        ...
                        <input type="number" name="ano" id="edit_ano" min="1900" max="{{ date('Y') + 1 }}"
                            class="mt-2 w-full rounded-lg border-gray-300 shadow-sm py-2 px-3">
                    </label>
                    ...
                    <label class="block">
                        ...
                        <select name="tipo_veiculo_id" id="edit_tipo"
                            class="mt-2 w-full rounded-lg border-gray-300 shadow-sm py-2 px-3">
                    ...
                    <label class="block">
                        ...
                        <select name="combustivel" id="edit_combustivel"
                            class="mt-2 w-full rounded-lg border-gray-300 shadow-sm py-2 px-3">
                            <option value="">-- selecione --</option>
                    ...
                    <label class="block">
                        ...
                        <input type="number" step="0.1" name="km_atual" id="edit_km"
                            class="mt-2 w-full rounded-lg border-gray-300 shadow-sm py-2 px-3">
                    </label>
