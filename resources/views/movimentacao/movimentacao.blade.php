...
<style>
    ...
    .mov-root * { box-sizing: border-box; }
    .mov-root {
    ...
    .mov-avatar.yellow { background: #fef3c7; }
    .mov-avatar.green  { background: #d1fae5; }
    .mov-title {
        font-family: 'Space Grotesk', sans-serif;
        font-size: 1.2rem; font-weight: 700;
        color: #111827; margin: 0;
    }
    .mov-meta { font-size: .8rem; color: #6b7280; margin-top: 2px; }
    .mov-meta span { color: #3b82f6; font-weight: 600; }
    .mov-badge {
        margin-left: auto;
        padding: .3rem .9rem;
        border-radius: 999px;
    ...
    .mov-badge.yellow { background: #fef3c7; color: #92400e; }
    .mov-badge.green  { background: #d1fae5; color: #065f46; }
    ...
    .timeline-fill.step0 { width: 0%; background: #f59e0b; }
    .timeline-fill.step1 { width: 50%; background: linear-gradient(90deg, #f59e0b, #10b981); }
    .timeline-fill.step2 { width: 100%; background: #10b981; }
    ...
    .tl-dot.active { border-color: #f59e0b; background: #fef3c7; box-shadow: 0 0 0 4px #fef3c740; }
    .tl-dot.pending { opacity: .5; }
    ...
    .tl-label.done   { color: #059669; }

    .tl-time {
        font-size: .65rem; color: #b0b7c3; margin-top: 1px;
    }
    ...
    .form-input.readonly-look {
        ...
        border-color: #f0f0f3; cursor: default;
    }
    .form-input::placeholder { color: #9ca3af; }

    textarea.form-input { resize: vertical; min-height: 80px; }

    .form-hint { font-size: .7rem; color: #9ca3af; margin-top: 3px; }
    ...
    .error-text { font-size: .78rem; color: #ef4444; margin-top: .3rem; }
    ...
    .form-divider span { font-size: .75rem; color: #9ca3af; white-space: nowrap; }
    .form-divider::before, .form-divider::after {
        content: ''; flex: 1; height: 1px; background: #f0f0f3;
    }
    ...
    .form-actions-hint { font-size: .75rem; color: #9ca3af; }
    ...
    .mobile-footer {
        position: fixed; inset-x: 0; bottom: 0; z-index: 50;
    ...
    .btn-mobile:active { transform: scale(.97); }
    .btn-mobile.yellow { background: #f59e0b; color: #fff; box-shadow: 0 2px 12px #f59e0b55; }
    .btn-mobile.green  { background: #10b981; color: #fff; box-shadow: 0 2px 12px #10b98155; }
    ...
    <div class="mov-card">
        ...
        <form id="movForm" method="POST"
            @if ($movimentacao->isEmpty())
                action="{{ route('movimentacao.store') }}"
            @else
                action="{{ route('movimentacao.update', $movimentacao->first()->id) }}"
            @endif
            ...
            <select name="veiculo_id" id="veiculo_id" hidden>
                @if ($movimentacao->isEmpty())
                    @foreach ($veiculos as $v)
                        <option selected value="{{ $v->id }}" data-combustivel="{{ $v->combustivel }}" data-km="{{ $v->km_atual }}">
                            {{ $v->placa }} - {{ $v->modelo }}
                        </option>
                    @endforeach
                @else
            ...
            <select name="user_id" id="motorista_id" hidden>
                ...
                @else
                    <option selected value="{{ $movimentacao->first()->user->id }}">{{ $movimentacao->first()->user->name }}</option>
                @endif
            </select>
            ...
            <div class="form-actions">
                <span class="form-actions-hint">Campos com <span style="color:#ef4444">*</span> são obrigatórios</span>
